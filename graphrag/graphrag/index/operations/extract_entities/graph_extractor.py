    ...
    async def _process_results(
        ...
        for source_doc_id, extracted_data in results.items():
            ...
            for record in records:
                ...
                if record_attributes[0] == '"entity"' and len(record_attributes) >= 7:
                    # add this record as a node in the G
                    entity_name = clean_str(record_attributes[1].upper())
                    entity_type = clean_str(record_attributes[2].upper())
                    entity_description = clean_str(record_attributes[3])
                    entity_location = clean_str(record_attributes[4])
                    entity_timestamp_start = clean_str(record_attributes[5])
                    entity_timestamp_end = clean_str(record_attributes[6])
                    ...
                    if entity_name in graph.nodes():
                        ...
                        node["location"] = (
                            entity_location if entity_location != "" else node["location"]
                        )
                        node["timestamp_start"] = (
                            node["timestamp_start"] if "timestamp_start" in node and node["timestamp_start"] != "null" else entity_timestamp_start
                        )
                        node["timestamp_end"] = (
                            node["timestamp_end"] if "timestamp_end" in node and node["timestamp_end"] != "null" else entity_timestamp_end
                        )
                    else:
                        graph.add_node(
                            entity_name,
                            ...
                            timestamp_start=entity_timestamp_start, 
                            timestamp_end=entity_timestamp_end
                        )
